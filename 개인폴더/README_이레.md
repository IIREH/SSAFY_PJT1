...
# 1월 24일
## annotation
- @Id: mongoDB의 _id field에 해당하며 ObjectId로 맞춰줄 수도 있고 사용자 정의로 값을 줄 수도 있고 객체 생성시 안넣어도 알아서 값 넣어줌
- @CreatedDate: 기본적으로 GMT를 기준으로 객체 생성시간을 자동으로 넣어줌
- @DBRef: 외부 Collection을 참조하는 field로 해당 Collection의 클래스에 해당하는 멤버변수를 넣어주면 됨

그 외에 아 이거 어노테이션으로 넣으면 좋을 거 같은데? 싶은 것들은 검색하면 어노테이션으로 찾을 수 있을 거 같다. 위의 것들도 그렇게 찾았고. 그간 어노테이션들 보면 xml의 순한맛처럼 어지럽고 조잡해보였는데 보다보니 괜찮다 싶다.

## Optioal <T>
일종의 Wrapper class로 null일 수도 있는 값을 관리하기 위해 쓰이며 따라서 null pointer exception의 위험을 줄이면서 null처리에 편리한 method들을 제공한다고 한다. 언어단에서 null 검사를 강요하니 실수할 일은 없고 데이터가 가지는 값도 명확히 전달되니 좋지 싶다. 

## id field에 값 할당
@Id 등으로 id 값이 자동으로 할당된는 경우 디버깅 모드로 단계마다 살펴보니 생성시엔 null이고 DB에 들어갈 때 비로소 값이 할당되더라. 당연한 말이지만 DB 들어가고 다시 null이 되는 것은 아니기에 다시 DB에 넣는다 해도 insert가 아닌 update 취급이며 완전 같은 값이니 실질적인 변화 x (참고로 MongoRepository에서 제공하는 insert는 요 인터페이스에서만 쓰이는 모양이다. 범용성을 위해선 MongoRepository가 상속받은 CrudRepository의 save를 사용하란다. 얘는 id 중복 없으면 indsert, 중복이면 update로 동작, 그래서 이름이 save)

## git branch naming
이제 슬슬 코드작성 단계로 넘어가며 마주한 생각못한 문제. master와 develop이라는 두 flow는 정했지만 develop에서 파생된 branch는 컨벤션을 정했나..? 내가 참여 못했던 회의에서 의논했었나..? 팀원 보니 그냥 feature라고만 했는데 branch가 개인에 따라 병렬적으로 진행되니 좀 더 명확한 이름이 필요하지 않을까 하는데 여기에 대해선 뭐가 정해졌는지 생각이 안난다. 그래서 commit도 못하고 애꿎은 README_이레.md만 작성!
